...
import { Button, Popover, RefSelectProps, Select } from 'antd';
        ...
        if (currentAccount !== null) {
          ...
          resolve(<Select ref={select} onChange={onClick} loading={isLoading} options={dropdownItems} bordered={false}
                          ...
                          dropdownRender={(dropdown) => (<>
                            {dropdown}
                            <Popover content={offlineMode ? 'Disabled in offline mode' : null} placement={'bottom'}>
                              <Button disabled={offlineMode} icon={<Icon icon={addIcon} className={styles.icon} />}
                                      onClick={add} className={styles.AddButton} />
                            </Popover>
                          </>)}
          />);
        } else {
